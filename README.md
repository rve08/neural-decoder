...
preferred dircetion and follows a roughly cosine-shaped relationship with movement angle (Georgopoulos, 1986).
...
Ridge regression derived from scratch and implemented using NumPy (np.linalg.solve, not explicit inversion here), and verified against
sklearn.linear_model.ridge. Lagged design matrix with K=10 lags across 96 neurons predicting x/y cursor velocity, chronological 
80/20 train/test split to avoid leakage.

**Results so far**
Wiener filter on test data: R² ≈ 0.44-0.61 for x/y-velocity
